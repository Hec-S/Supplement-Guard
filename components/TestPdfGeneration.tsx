...
import { ClaimData, ComparisonAnalysis, RiskLevel, SeverityLevel, VarianceType, CostCategory } from '../types';
...
The dramatic increase in both labor rates and scope of work, combined with the addition of numerous high-value operations, presents significant fraud risk indicators that warrant detailed investigation and verification of all charges.`,
  ...
  // Sample comparison analysis data for enhanced PDF
  const sampleComparisonAnalysis: ComparisonAnalysis = {
    analysisId: 'CLM-2025-080512',
    timestamp: new Date(),
    statistics: {
      totalVariance: 3069.18,
      totalVariancePercent: 252.6,
      itemCount: 23,
      averageVariance: 133.44,
      medianVariance: 48.00,
      standardDeviation: 245.67,
      varianceRange: { min: -163.50, max: 1108.80 },
      categoryVariances: {
        [CostCategory.LABOR]: { variance: 2145.30, variancePercent: 185.2, itemCount: 12 },
        [CostCategory.PARTS]: { variance: 456.78, variancePercent: 89.3, itemCount: 6 },
        [CostCategory.MATERIALS]: { variance: 234.56, variancePercent: 45.7, itemCount: 3 },
        [CostCategory.OTHER]: { variance: 232.54, variancePercent: 78.9, itemCount: 2 }
      },
      varianceTypeDistribution: {
        [VarianceType.NEW_ITEM]: { count: 15, totalAmount: 2456.78, percentage: 65.2 },
        [VarianceType.PRICE_CHANGE]: { count: 3, totalAmount: 789.45, percentage: 13.0 },
        [VarianceType.QUANTITY_CHANGE]: { count: 2, totalAmount: 234.56, percentage: 8.7 },
        [VarianceType.DESCRIPTION_CHANGE]: { count: 1, totalAmount: 123.45, percentage: 4.3 },
        [VarianceType.REMOVED_ITEM]: { count: 0, totalAmount: 0, percentage: 0 },
        [VarianceType.NO_CHANGE]: { count: 2, totalAmount: 0, percentage: 8.7 }
      },
      dataQuality: {
        completeness: 0.95,
        consistency: 0.87,
      ...
      suspiciousPatterns: [
        {
          description: 'Labor rate increase of 79% without justification',
          confidence: 0.95,
          severity: SeverityLevel.HIGH
        },
        {
          description: 'Scope creep with 15 new line items added',
          confidence: 0.88,
    ...
    riskAssessment: {
      ...
      riskFactors: [
        {
          description: 'Excessive labor rate increase',
          impact: 0.35,
          confidence: 0.95
        },
        {
          description: 'Significant scope expansion',
          impact: 0.28,
          confidence: 0.88
        },
        {
          description: 'Addition of premium services',
          impact: 0.22,
          confidence: 0.76
        }
      ],
      recommendations: [
        'Verify labor rate justification with market analysis',
        'Review all new line items for necessity and accuracy',
        'Conduct on-site inspection to validate scope changes',
        'Request detailed documentation for all premium services'
      ]
    },
    discrepancies: [
      {
        description: 'Labor rate increase without documentation',
        detailedExplanation: 'Labor rates increased from $67/hr to $120/hr (79% increase) without proper market justification or documentation.',
        severity: SeverityLevel.HIGH,
        recommendedAction: 'Request market analysis and rate justification documentation'
      },
      {
        description: 'Scope creep with multiple new operations',
        detailedExplanation: 'Original estimate for bumper repair expanded to include roof, rocker, and quarter panel work without clear damage assessment.',
        severity: SeverityLevel.MEDIUM,
        recommendedAction: 'Conduct independent damage assessment to verify necessity of additional work'
      }
    ]
  };
  ...
  const handleGenerateEnhancedPdf = async () => {
    setIsGenerating(true);
    try {
      const blob = await enhancedPdfService.generateComprehensiveReport(sampleComparisonAnalysis);
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `Enhanced-Claim-Report-${sampleComparisonAnalysis.analysisId}.pdf`;
      link.click();
      URL.revokeObjectURL(url);
      setLastGenerated('Enhanced PDF (Fixed)');
    } catch (error) {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <button
              ...
              disabled={isGenerating}
              className="flex flex-col items-center p-4 bg-purple-50 border border-purple-200 rounded-lg hover:bg-purple-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <div className="text-2xl mb-2">📊</div>
              <div className="font-medium text-purple-800">Enhanced PDF (Fixed)</div>
              <div className="text-sm text-purple-600 text-center mt-1">
                Fixed comprehensive analysis PDF
              </div>
            </button>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              <ul className="text-sm text-slate-600 space-y-2">
                <li>• Increased safe margins (25mm → 30mm)</li>
